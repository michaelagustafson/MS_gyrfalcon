...
sq.coef <- as.data.frame(coef(fm.4))
write.csv(sq.coef, here("output/sqcoef.csv"))
...
write.csv(sq.state.confint, here("output/sqci_state.csv"))
write.csv(sq.det.confint, here("output/sqci_det.csv"))
...
ggsave("SquirXElev2.png", width = 7.5, height = 7.5, dpi = 300)
...
ggsave("SquirXLow2.png", width = 7.5, height = 7.5, dpi = 300)
...
s.svp <- cbind( pred.sv[,c("Predicted", "lower", "upper") ], sparseveg.pp ) %>%
 ...
 # ylim(0, 0.35) +
  theme_bw( base_size = 50 ) +
  # add labels
  labs( x = "Sparse veg (%)", y = "Arctic ground squirrel \n relative abundance") + 
  # add band of confidence intervals
  #theme(axis.title.y=element_blank()) +
  theme(text=element_text(size=60)) +
  geom_smooth( aes(ymin = lower, ymax = upper ), 
...
ggsave("SquirXSparse3.png", width = 7.5, height = 7.5, dpi = 300)
