...
s.svp <- cbind( pred.sv[,c("Predicted", "lower", "upper") ], sparseveg.pp ) %>%
  ...
  scale_x_continuous(labels = scales::percent_format(accuracy=1)) +
  # add labels
  labs( x = "Sparse vegetation", y = "Arctic ground squirrel density") + 
  # add band of confidence intervals
  #theme(axis.title.y=element_blank()) +
  theme(text=element_text(size=25)) +
  geom_smooth( aes(ymin = lower, ymax = upper ), 
...
ggsave("E:/gyrf_analysis/partial prediction plots/squir_sparseveg.png", dpi = 300)
...
obsvp.det <- pred.obs %>%
  ...
  geom_errorbar( aes(ymin = lower, ymax = upper ), 
               size = 1.5, width = 0.3 ) +
  scale_color_manual(values=c('tomato','brown3','firebrick')) +
  ylim(0,1.0)
#view
obsvp.det


ggsave("E:/gyrf_analysis/detection partial plots/squir_observer.png", dpi = 300)
```
Julian
...
julp.det <- cbind( pred.jul[,c("Predicted", "lower", "upper") ], julian.pp ) %>%
  ...
  geom_smooth( aes(ymin = lower, ymax = upper ), 
               stat = "identity",
               size = 1.5, alpha = 0.5, fill = "lightsalmon" ) +
  # add mean line on top
  geom_line( size = 2, color = "darkred" ) +
  ylim(0,1.0)
#view
...
ggsave("E:/gyrf_analysis/detection partial plots/squir_julian.png", dpi = 300)
...
ggsave("E:/gyrf_analysis/detection partial plots/squir_wind.png", dpi = 300)
...
ggsave("E:/gyrf_analysis/detection partial plots/squir_minsun.png", dpi = 300)
