...
sum(wipt2$count) # sample size 518
...
#write.csv(wipt.wide, here("output/wiptwide.csv"))
wipt.wide <- read.csv(here("output/wiptwide.csv"))
...
wp.coef <- as.data.frame(coef(fm.2))
wp.state.confint <- as.data.frame(confint(fm.2, level = 0.95, type = "state"))
wp.det.confint <- as.data.frame(confint(fm.2, level = 0.95, type = "det"))
write.csv(wp.coef, here("output/wpcoef.csv"))
write.csv(wp.state.confint, here("output/wpci_state.csv"))
write.csv(wp.det.confint, here("output/wpci_det.csv"))
...
area.raster <- elev 
values(area.raster) <- 400*400/10000 # area of a grid pixel
...
save.image(plot(wipt.prediction, axes=FALSE, col=terrain.colors(100)))
plot(wipt.prediction, axes=FALSE, col=terrain.colors(100))
...
w.elevp <- cbind( pred.elev[,c("Predicted", "lower", "upper") ], elevation.pp ) %>%
  ...
  geom_smooth( aes(ymin = lower, ymax = upper ), 
               stat = "identity",
               size = 1.5, alpha = 0.5, fill = "lightblue3" ) +
  # add mean line on top
  geom_line( size = 2, color = "darkblue" )
#view
w.elevp
ggsave("E:/gyrf_analysis/partial prediction plots/wipt_elev.png", dpi = 300)
...
#view
w.tussp
ggsave("E:/gyrf_analysis/partial prediction plots/wipt_tussock.png", dpi = 300)
...
#view
w.tundp
ggsave("E:/gyrf_analysis/partial prediction plots/wipt_tundra2.png", dpi = 300)
...
ggsave("E:/gyrf_analysis/partial prediction plots/wipt_allshrub2.png", dpi = 300)
...
obsvp.det <- pred.obs %>%
  ...
  geom_errorbar( aes(ymin = lower, ymax = upper ), 
               size = 1.5, width = 0.3 ) +
  scale_color_manual(values=c('deepskyblue4','steelblue4','royalblue4')) +
  ylim(0,1.0)
#view
obsvp.det


ggsave("E:/gyrf_analysis/detection partial plots/wipt_observer.png", dpi = 300)
```
Julian
...
ggsave("E:/gyrf_analysis/detection partial plots/wipt_julian.png", dpi = 300)
...
windp.det <- cbind( pred.wind[,c("Predicted", "lower", "upper") ], wind.pp ) %>%
  # define x and y values
  ggplot(., aes( x = wind.pp, y = Predicted ) ) + 
  #choose preset look
  theme_classic( base_size = 15 ) +
  # add labels
  labs( x = "Wind speed (mph)", y = "Predicted detection" ) +
  # add band of confidence intervals
  geom_smooth( aes(ymin = lower, ymax = upper ), 
               stat = "identity",
               size = 1.5, alpha = 0.5, fill = "lightblue3" ) +
...
ggsave("E:/gyrf_analysis/detection partial plots/wipt_wind.png", dpi = 300)
...
ggsave("E:/gyrf_analysis/detection partial plots/wipt_minsun.png", dpi = 300)
