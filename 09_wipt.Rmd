...
wipt.prediction <- predict(fm.2, type="state", newdata=pred.surface)
...
write.csv(wipt.freq1, here("output/wiptcellcounts.csv"))
...
pred.elev <- predict( fm.2, type = "state", newdata = abundData, 
                          appendData = TRUE )
#view
...
#view
w.elevp
ggsave("partial prediction plots/wipt_elev.png", dpi = 300)
...
#predict partial relationship:
pred.tuss <- predict( fm.2, type = "state", newdata = abundData, 
                          appendData = TRUE )
#view
...
w.tussp <- cbind( pred.tuss[,c("Predicted", "lower", "upper") ], tussock.pp ) %>%
  ...
  labs( x = "Tussock", y = "Willow Ptarmigan \n relative abundance" ) +
  # add band of confidence intervals
  theme(text=element_text(size=25)) +
  geom_smooth( aes(ymin = lower, ymax = upper ), 
               stat = "identity",
...
#view
w.tussp
ggsave("partial prediction plots/wipt_tussock.png", dpi = 300)
...
#predict partial relationship:
pred.tun <- predict( fm.2, type = "state", newdata = abundData, 
                          appendData = TRUE )
#view
...
w.tundp <- cbind( pred.tun[,c("Predicted", "lower", "upper") ], tundra.pp ) %>%
  ...
  scale_x_continuous(labels = scales::percent_format(accuracy=1)) +
  # add labels
  labs( x = "Tundra", y = "Willow Ptarmigan \n relative abundance" ) +
  # add band of confidence intervals
  theme(text=element_text(size=25)) +
  geom_smooth( aes(ymin = lower, ymax = upper ), 
               stat = "identity",
...
#view
w.tundp
ggsave("partial prediction plots/wipt_tundra.png", dpi = 300)
...
#predict partial relationship:
pred.alls <- predict( fm.2, type = "state", newdata = abundData, 
                          appendData = TRUE )
#view
...
w.allsp <- cbind( pred.alls[,c("Predicted", "lower", "upper") ], allshrub.pp ) %>%
  ...
  scale_x_continuous(labels = scales::percent_format(accuracy=1)) +
  # add labels
  labs( x = "All shrub", y = "Willow Ptarmigan \n relative abundance" ) +
  # add band of confidence intervals
  theme(text=element_text(size= 25)) +
  geom_smooth( aes(ymin = lower, ymax = upper ), 
               stat = "identity",
...
ggsave("partial prediction plots/wipt_allshrub.png", dpi = 300)
