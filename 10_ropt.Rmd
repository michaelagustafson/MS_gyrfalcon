...
area.raster <- elev 
values(area.raster) <- 400*400/10000 # area of a grid pixel then divide by 10000 to make a smaller number and standardize the same as the area of the sampled area in the model
...
ggsave("E:/gyrf_analysis/partial prediction plots/ropt_sparseveg.png", dpi = 300)
...
obsvp.det <- pred.obs %>%
  ...
  geom_errorbar( aes(ymin = lower, ymax = upper ), 
               size = 1.5, width = 0.3 ) +
  scale_color_manual(values=c('palegreen4','springgreen4','darkgreen')) +
  ylim(0,1.0)
#view
obsvp.det


ggsave("E:/gyrf_analysis/detection partial plots/ropt_observer.png", dpi = 300)
```
Julian
...
julp.det <- cbind( pred.jul[,c("Predicted", "lower", "upper") ], julian.pp ) %>%
  ...
  geom_smooth( aes(ymin = lower, ymax = upper ), 
               stat = "identity",
               size = 1.5, alpha = 0.5, fill = "darkseagreen3" ) +
  # add mean line on top
...
ggsave("E:/gyrf_analysis/detection partial plots/ropt_julian.png", dpi = 300)
...
ggsave("E:/gyrf_analysis/detection partial plots/ropt_wind.png", dpi = 300)
...
ggsave("E:/gyrf_analysis/detection partial plots/ropt_minsun.png", dpi = 300)
