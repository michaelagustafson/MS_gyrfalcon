...
values(jul.raster) <- mean(scaled$julian) # replace data with that of what you want - doing it with the mean scaled value of obs covs

wind.raster <- elev 
values(wind.raster) <- mean(scaled$wind)

minsun.raster <- elev 
values(minsun.raster) <- mean(scaled$min_after_sun)
...
#look at some stats
cellStats(p.prediction, "sum") # 2 755 724.6
cellStats(p.prediction, "min") # minimum density of 20.5 squirrels/km2
cellStats(p.prediction, "max") # maximum density of 3 339 squirrels/km2
...
#look at some stats
cellStats(zip.prediction, "sum") # 18 778 386 mil
cellStats(zip.prediction, "min") # minimum density of 321 squirrels /km2
cellStats(zip.prediction, "max") # maximum density of 8 336 squirrels/km2
...
tund.p <- cbind( tun.p[,c("Predicted", "lower", "upper") ], tundra.pp ) %>%
  ...
  # add labels
  labs( x = "Tundra", y = "Relative density (squirrels/kmsq)" ) +
...
ggsave("E:/gyrf_analysis/partial prediction plots 14Nov23/dist_squir_tundra.png", width = 20, height = 15, dpi = 300, limitsize = FALSE)
...
p.veg <- cbind( veg.p[,c("Predicted", "lower", "upper") ], sparse.pp ) %>%
  ...
  # turn x axis into percent scale
  scale_x_continuous(labels = scales::percent_format(accuracy=1)) +
  labs( x = "Sparse Vegetation", y = "Relative density (squirrels/kmsq)" ) +
  # add band of confidence intervals
  theme(text=element_text(size=65)) +
  geom_smooth( aes(ymin = lower, ymax = upper ), 
               stat = "identity",
...
ggsave("E:/gyrf_analysis/partial prediction plots 14Nov23/dist_squir_sparse.png", width = 20, height = 15, dpi = 300, limitsize = FALSE)
...
p.low <- cbind( low.p[,c("Predicted", "lower", "upper") ], lowshrub.pp ) %>%
  ...
  scale_x_continuous(labels = scales::percent_format(accuracy=1)) +
  # add labels
  labs( x = "Low shrub", y = "Relative density (squirrels/kmsq)" ) +
  # add band of confidence intervals
  theme(text=element_text(size= 65)) +
  geom_smooth( aes(ymin = lower, ymax = upper ), 
               stat = "identity",
...
ggsave("E:/gyrf_analysis/partial prediction plots 14Nov23/dist_squir_lowshrub.png", width = 20, height = 15, dpi = 300, limitsize = FALSE)
```
...
elev.p <- cbind( pred.elev.p[,c("Predicted", "lower", "upper") ], elevation.pp ) %>%
  ...
  # add labels
 # ylim(0, 13) +
  labs( x = "Elevation (m)", y = "Relative density (squirrels/kmsq)" ) +
  theme(text=element_text(size=65)) +
    # add band of confidence intervals
  geom_smooth( aes(ymin = lower, ymax = upper ), 
               stat = "identity",
...
ggsave("E:/gyrf_analysis/partial prediction plots 14Nov23/dist_squir_elevation.png", width = 20, height = 15, dpi = 300, limitsize = FALSE)
```
...
ggsave("E:/gyrf_analysis/partial prediction plots 14Nov23/dist_squir_tundra_zip.png", width = 20, height = 15, dpi = 300, limitsize = FALSE)
```
...
ggsave("E:/gyrf_analysis/partial prediction plots 14Nov23/dist_squir_sparse_zip.png", width = 20, height = 15, dpi = 300, limitsize = FALSE)
```
...
ggsave("E:/gyrf_analysis/partial prediction plots 14Nov23/dist_squir_lowshrub_zip.png", width = 20, height = 15, dpi = 300, limitsize = FALSE)
```
...
ggsave("E:/gyrf_analysis/partial prediction plots 14Nov23/dist_squir_elevation_zip.png", width = 20, height = 15, dpi = 300, limitsize = FALSE)
```
