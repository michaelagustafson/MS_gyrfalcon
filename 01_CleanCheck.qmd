...
wipt.male.agg <- aggregate(x = wipt.male2$count, 
                      by = list(wipt.male2$id, wipt.male2$time_int), 
                      FUN = sum)

names(wipt.male.agg)[names(wipt.male.agg) == "Group.1"] <- "id"
names(wipt.male.agg)[names(wipt.male.agg) == "Group.2"] <- "time_int"
names(wipt.male.agg)[names(wipt.male.agg) == "x"] <- "count"

# checks for missing data

all.env2$id <- as.factor(all.env2$id)
miss1 <- anti_join(all.env2, wipt.male.agg) # clear
miss2 <- anti_join(wipt.male.agg, all.env2) # clear
...
unique(wipt.male.agg$id) # should end up with 988 levels

wipt.male.wide <- wipt.male.agg %>%
  dplyr::select(id, time_int, count) %>%
  spread(key = time_int, value = count, fill = 0)

# double check that sample size is still 485
sum(wipt.male.wide[,c(2:6)]) # sweet! good to go
...
jul.scaled <- scale(as.numeric(all.env2$julian))
min.scaled <- scale(as.numeric(all.env2$min_after_sun))
...
# test correlation
cor.mat <- Hmisc::rcorr(as.matrix(all.env.scaled[,c(2:16, 23, 27:28)]),type="pearson")
cor.mat
```
...
wipt.male.env.join <- left_join(wipt.male.wide, all.env.scaled)
head(wipt.male.env.join); dim(wipt.male.env.join)
str(wipt.male.env.join)

write.csv(wipt.male.env.join, "E:/gyrf_analysis/MSgyrfalcon/MSgyrfalcon/wipt_male_df_18Oct23.csv")
```
